...
def send_guvi_callback(session_id: str, session: dict, serializable_intel: dict) -> bool:
...
async def receive_message(
    ...
    try:
        ...
        # Detect scam - FIX: returns dict not tuple
        ...
        if scam_type and not session["scam_type"]:
        ...
        # Merge extracted intelligence into session (avoid duplicates)
        for key, values in extracted.items():
            if key in session["intelligence"]:
                # Convert list to set, update, convert back
        ...
        # Generate AI reply - FIX: correct parameters
        try:
        ...
        # Send GUVI callback if conditions met
        # Callback sent after sufficient engagement (turns >= 3) and scam detected
        should_send_callback = (
            session["is_scam"] 
            and session["turns"] >= 3  # At least 3 exchanges
            and not session.get("callback_sent", False)
            and len(serializable_intel.get("bankAccounts", []) + 
                   serializable_intel.get("upiIds", []) + 
                   serializable_intel.get("phishingLinks", [])) > 0  # Some intelligence extracted
        )
        
        if should_send_callback:
            print(f"📞 Sending callback for session {session_id}")
            success = send_guvi_callback(session_id, session, serializable_intel)
            if success:
                session["callback_sent"] = True
                print(f"✅ Callback sent successfully")
        
        # Return response matching hackathon format
        response_data = {
            "status": "success",
            "reply": reply,
            "scamDetected": session["is_scam"],
            "scamScore": session["scam_score"],
            "scamType": session.get("scam_type"),  # Use .get() to handle None
            "extractedIntelligence": serializable_intel,
            "engagementMetrics": {
                "totalMessagesExchanged": session["turns"],
